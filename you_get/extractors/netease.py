...
def netease_cloud_music_download(url, output_dir='.', merge=True, info_only=False, **kwargs):
    ...
    if "album" in url:
        ...
        if not info_only:
            ...
            download_urls([cover_url], "cover", "jpg", 0, new_dir, **kwargs)

        for i in j['album']['songs']:
            netease_song_download(i, output_dir=new_dir, info_only=info_only, **kwargs)
            try: # download lyrics
                ...
                netease_lyric_download(i, l["lrc"]["lyric"], output_dir=new_dir, info_only=info_only, **kwargs)
            except: pass
...
def netease_download(url, output_dir = '.', merge = True, info_only = False, **kwargs):
    ...
    if "music.163.com" in url:
        return netease_cloud_music_download(url, output_dir, merge, info_only, **kwargs)
    else:
        ...
        if not info_only:
            return download_urls([url], title, ext, size, output_dir = output_dir, merge = merge, **kwargs)
