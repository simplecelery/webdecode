...
def tumblr_download(url, output_dir='.', merge=True, info_only=False, **kwargs):
    if re.match(r'https?://\d+\.media\.tumblr\.com/', url):
        return universal_download(url, output_dir, merge=merge, info_only=info_only, **kwargs)
        return
        ...
        if tuggles:
            ...
            if not info_only:
                for t in tuggles:
                    ...
                    return download_urls([url], title, ext, size,
    ...
    if not real_url:
        ...
        if iframe_url:
            ...
            if re.search(r'player\.vimeo\.com', iframe_url):
                return vimeo_download(iframe_url, output_dir, merge=merge, info_only=info_only,
                               referer='http://tumblr.com/', **kwargs)
            elif re.search(r'dailymotion\.com', iframe_url):
                return dailymotion_download(iframe_url, output_dir, merge=merge, info_only=info_only, **kwargs)
                
            elif re.search(r'vine\.co', iframe_url):
                return vine_download(iframe_url, output_dir, merge=merge, info_only=info_only, **kwargs)
    ...
    if not info_only:
        return download_urls([real_url], title, ext, size, output_dir, merge=merge, **kwargs)
