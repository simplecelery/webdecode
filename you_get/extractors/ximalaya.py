...
def ximalaya_download_by_id(id, title = None, output_dir = '.', info_only = False, stream_id = None, **kwargs):
...
#no size data in the json. should it be calculated?
    ...
    if not info_only:
        ...
        print('Size:        N/A')
        return download_urls(urls, title, ext, size, output_dir = output_dir, merge = False, **kwargs)
...
def ximalaya_download(url, output_dir = '.', info_only = False, stream_id = None, **kwargs):
    ...
    return ximalaya_download_by_id(id, output_dir = output_dir, info_only = info_only, stream_id = stream_id, **kwargs)
