...
def longzhu_download(url, output_dir = '.', merge=True, info_only=False, **kwargs):
    ...
    if (web_domain == 'star.longzhu.com') or (web_domain == 'y.longzhu.com'):
        ...
        if not info_only:
            return download_urls([real_url], title, 'flv', None, output_dir, merge=merge, **kwargs)
        ...
        if player:
            ...
            download_urls([real_url], title, 'm3u8', 0, output_dir, merge=merge, **kwargs)
        else:
            urls = general_m3u8_extractor(real_url)
            print_info('Longzhu Video', title, 'm3u8', 0)
            if not info_only:
                return download_urls(urls, title, 'ts', 0, output_dir=output_dir, merge=merge, **kwargs)
