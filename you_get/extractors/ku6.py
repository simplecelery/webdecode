...
def ku6_download_by_id(id, title = None, output_dir = '.', merge = True, info_only = False, **kwargs):
    ...
    if not info_only:
        return download_urls(urls, title, ext, size, output_dir, merge = merge, **kwargs)
...
def ku6_download(url, output_dir = '.', merge = True, info_only = False, **kwargs):
    ...
    if id is None:
        ...
        if not info_only:
            return download_urls([video_url], title, 'mp4', video_size, output_dir, merge=merge, **kwargs)
        return

    ku6_download_by_id(id, output_dir = output_dir, merge = merge, info_only = info_only, **kwargs)
