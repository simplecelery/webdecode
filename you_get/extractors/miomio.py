...
def miomio_download(url, output_dir = '.', merge = True, info_only = False, **kwargs):
    ...
    if t == 'youku':
        return youku_download_by_vid(id, title=title, output_dir=output_dir, merge=merge, info_only=info_only, **kwargs)
    elif t == 'tudou':
        ...
        if not info_only:
            return download_urls(url_list, title, ext, total_size=size_full, output_dir=output_dir, merge=merge, headers=fake_headers, **kwargs)
    else:
